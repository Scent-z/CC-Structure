"""
TodoWrite — add a planning tool on top of s04 hooks.
...
def _normalize_todos(todos):
    if isinstance(todos, str):
        try:
            ...
            try:
                todos = ast.literal_eval(todos)  # 将字符串转换为 Python 对象
...
def agent_loop(messages: list):
    ...
    while True:
        # 如果模型连续 3 轮没有更新 todos，就自动插入一条提醒，让模型去更新任务列表
        ...
        for block in response.content:
            ...
            # 当 todo_write 被调用时，把 nag counter（催更计数器）重置为 0
