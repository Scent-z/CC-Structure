#!/usr/bin/env python3
"""
子代理（Subagent）—— 使用全新的 messages[] 消息上下文创建子代理，以实现上下文隔离
...
# 这里提示词感觉应该再加一点, 怕模型反应不过来
# 拿前一节的 system prompt来换, 强调主代理在进行多步任务前都要使用todo_write规划每一步的任务, 同时给子代理派任务时每个子代理应该只派一个
SYSTEM = (
...
# ═══════════════════════════════════════════════════════════
#  NEW in s06: subagent gets its own system prompt — no task, no recursion
# ═══════════════════════════════════════════════════════════
SUB_SYSTEM = (
    f"You are a coding agent at {WORKDIR}. "
...
# ✅️
# 子代理循环 (工具7 分派子任务) (主Agent独享工具)
# 不给子代理task工具, 防止子代理也递归地创建新的子代理
def spawn_subagent(description: str) -> str:
    """使用新消息[]交给子代理, 子代理仅返回摘要。"""
    print(f"\n\033[35m[Subagent spawned]\033[0m")
    messages = [{"role": "user", "content": description}]  # fresh context

    for _ in range(30):  # 限制一下子代理循环次数, safety limit
    ...
    if not result:
        # 最后一条消息是 tool_result, 向前查找 assistant 的文本
