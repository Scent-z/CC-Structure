"""
Tool Use — 在 s01 基础上新增 4 个工具 + 分发映射。
...
def safe_path(p: str) -> Path:
    ...
    if not path.is_relative_to(WORKDIR):  # 一个路径是否位于另一个路径内部（是否是其子路径）
...
def run_write(path: str, content: str) -> str:
    try:
        ...
        file_path.parent.mkdir(parents=True, exist_ok=True)  # parents=True表示允许递归创建
